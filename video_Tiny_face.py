...
def cut_bounding_boxes(raw_img, refined_bboxes, output_dir):
    ...
    for r in refined_bboxes:
        ...
        # cv2.cvtColor(cropimage, cv2.COLOR_RGB2BGR)
        cropimagename = os.path.join(output_dir, str(_r[0]) + '.png')

        min_x = min(_r[0] - 5, _r[2] + 5)
        min_y = min(_r[1] - 5, _r[3] + 5)
        width = abs(_r[0] - _r[2]) + 10
        height = abs(_r[1] - _r[3]) + 10
        if (width > 31 and height > 31):
            cut_img = raw_img[min_y:min_y + height, min_x:min_x + width]
            cv2.imwrite(cropimagename, cut_img)
...
def evaluate(weight_file_path, data_dir, output_dir, prob_thresh=0.5, nms_thresh=0.1, lw=3, display=False):
    ...
    with tf.Session(config=config) as sess:
        ...
        for filename in filenames:
            ...
            video_writer = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc('M', 'P', 'E', 'G'), fps, out_video_size)
            ...
            for index in test_bar:
                if success:
                    if (index % per_fps == 0):
                        ...
                        cut_bounding_boxes(cut_img, refined_bboxes, output_dir+'/'+fname.split('.')[0]+'/'+str(index*per_fps))
...
def main():
    ...
    argparse.add_argument('--data_dir', type=str, help='Image data directory.', default="./input_videos")
